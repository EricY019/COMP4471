...
class CoattentionModel(nn.Module):  # spatial and channel attention module
    def __init__(self, num_classes=1, all_channel=2048, all_dim=14 * 14):  # 473./8=60 416./8=52
        # all_channel = C ,all_dim = N * N, size of feature map
        ...
        self.globalAvgPool = nn.AvgPool2d(14, stride=1) # TO-BE MODIFIED - N
        self.fc1 = nn.Linear(in_features=2048 * 2, out_features=16)
        self.fc2 = nn.Linear(in_features=16, out_features=2048)
        self.fc3 = nn.Linear(in_features=2048 * 2, out_features=16)
        self.fc4 = nn.Linear(in_features=16, out_features=2048)
        self.relu = nn.ReLU(inplace=True)
        self.sigmoid = nn.Sigmoid()

    def forward(self, exemplar, query):
        # spatial co-attention
        exemplar = exemplar.contiguous().view(8, 2048, 14, 14)
        query = query.contiguous().view(8, 2048, 14, 14)

        fea_size = query.size()[2:]
        all_dim = fea_size[0] * fea_size[1]
        exemplar_flat = exemplar.contiguous().view(-1, query.size()[1], all_dim)  # N,C,H*W
        query_flat = query.contiguous().view(-1, query.size()[1], all_dim)
        ...
        input1_att = exemplar_att.contiguous().view(-1, query.size()[1], fea_size[0], fea_size[1])
        input2_att = query_att.contiguous().view(-1, query.size()[1], fea_size[0], fea_size[1])
